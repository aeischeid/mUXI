...
window.muxi = {
    ...
    req.onreadystatechange = function() {
      if (req.readyState == 4) {
        if (replace)
          ...
          muxi.insertPages(frag.childNodes);
        }
        if (cb)
  ...
  isNativeUrl: function(href) {
    for(var i = 0; i < muxi.nativeUrlPatterns.length; i++) {
      if(href.match(muxi.nativeUrlPatterns[i])) return true;
    }
    return false;
  },
  nativeUrlPatterns: [
...
addEventListener("load", function(event) {
  var page = muxi.getSelectedPage();
  var locPage = getPageFromLoc();
  if (page)
      muxi.showPage(page);
  if (locPage && (locPage != page))
    muxi.showPage(locPage);
...
addEventListener("click", function(event) {
  ...
  if (link) {
    ...
    if (link.href && link.hash && link.hash != "#" && !link.target) {
      ...
      muxi.showPage($(link.hash.substr(1)));
      ...
      muxi.showPageByHref(link.href, null, null, link, unselect);
    }
    else if (muxi.isNativeUrl(link.href)) {
      return;
    }
      ...
      link.setAttribute("selected", "progress");
      muxi.showPageByHref(link.href, null, null, null, unselect);
    }
    else
      return;
...
function canDoSlideAnim() {
  return (muxi.animOn) && (typeof WebKitCSSMatrix == "object");
}
...
function submitForm(form) {
  muxi.showPageByHref(form.action || "POST", encodeForm(form), form.method);
}
